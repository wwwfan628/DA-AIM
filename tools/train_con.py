...
def match_contrasive(features_source, labels, feature_target):
    """
    features_source: source features num_boxes x (2048+256)
    labels: sopurce labels: num_boxes
    """
    ## implement here
    return None
...
def train_epoch(aux_train_loader, train_loader, model, optimizer,  train_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            preds, roi_features = model(global_input, global_boxes, head_type='action')
        ...
        features_source = roi_features[:num_ac_boxes,:]
        feature_target = roi_features[num_ac_boxes:,:]
        loss_aux = match_contrasive(features_source, labels, feature_target)
        # loss_aux = loss_fun_aux(preds_aux, labels_aux)
        misc.check_nan_losses(loss_aux)
        
        loss_action = loss_action*(1-cfg.CON.LOSS_FACTOR)
        loss_aux =  cfg.CON.LOSS_FACTOR*loss_aux 
