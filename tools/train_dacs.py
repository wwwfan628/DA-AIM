...
from slowfast.utils.dacs_helper import mix, use_pseudo_labels, plot_inputs
...
def train_epoch(aux_train_loader, train_loader, model, ema_model, optimizer, train_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        with torch.autograd.set_detect_anomaly(True):
            ...
            if cfg.DACS.CONSISTENCY_LOSS == 'ce_weighted':
                ...
                thresholds = torch.tensor(cfg.DACS.THRESHOLDS).unsqueeze(dim=0).repeat_interleave(len(pseudo_logits_unlabeled), dim=0).cuda()
            ...
            if cfg.DACS.AUGMENTATION_ENABLE:
            ...
            if cfg.TENSORBOARD.DACS.PLOT_REAL_SAMPLES and (writer is not None):
                if isinstance(inputs, (list,)):
            ...
            loss_fun_unlabeled = losses.get_loss_func(cfg.DACS.CONSISTENCY_LOSS)().cuda()
    ...
    if writer is not None and cfg.TENSORBOARD.DACS.PSEUDOLABELS_CONFUSION_MATRIX:
