...
def train_epoch(aux_train_loader, train_loader, model, optimizer,  train_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        # print(inputs[0].shape,meta["boxes"].shape, inputs_aux[1].shape, meta_aux["boxes"].shape)
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.MODEL.LOSS_FUNC == 'cross_entropy':
                preds, preds_aux = model(global_input, global_boxes, head_type='ce_aux_action')
            else:
                preds, preds_aux = model(global_input, global_boxes, head_type='aux_action')
        else:
        ...
        preds_aux = preds_aux[num_ac_boxes:, :]
        # preds_aux = model(inputs_aux, meta_aux["boxes"], "aux")
        # print(preds.shape, preds_aux.shape, labels.shape, labels_aux.shape)
        ...
        labels_aux = labels_aux.long()
        labels_aux = torch.argmax(labels_aux, dim=1)
        ## 0.3651408851146698 0.1804339736700058 after 280
        # append predictions and ground truth to list
        for i in range(labels_aux.shape[0]):
            # y_true.append(int(labels_aux[i]))
            # y_pred.append(int(torch.argmax(preds_aux[i, :])))
            y_true_interim.append(int(labels_aux[i]))
            y_pred_interim.append(int(torch.argmax(preds_aux[i, :])))
        ...
        if cfg.AUX.F_CONFUSION != -1 and (cur_iter % cfg.AUX.F_CONFUSION == 0 or cur_iter == aux_data_len-2):  # was 500 previously (also once it was 100)

            cm = create_cm(y_true_interim, y_pred_interim, cfg.AUX.CLASSES)
            plot_aux_confusion_matrix(cm=cm, normalize=True, target_names=[str(ii) for ii in range(cfg.AUX.CLASSES)],
            ...
            index = cur_iter / cfg.AUX.F_CONFUSION  # was 500 previously
