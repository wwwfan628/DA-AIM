    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.MODEL.LOSS_FUNC == 'cross_entropy':
                preds_grl, preds = model(global_input, bboxes_source=meta["boxes"], bboxes_target=meta_aux["boxes"], head_type='ce_grl_action')  # without softmax, raw logits
            else:
                preds_grl, preds = model(global_input, bboxes_source=meta["boxes"], bboxes_target=meta_aux["boxes"], head_type='grl_action')
        ...
        if cfg.MODEL.LOSS_FUNC == 'cross_entropy':
            ...
            loss_action += loss_fun_act(preds, labels_ce)
        else:
            loss_action += loss_fun_act(preds, labels)

        loss_fun_grl = losses.get_loss_func('cross_entropy')(reduction='mean')
        loss_grl = 0
        preds_source = preds_grl[0]
        preds_target = preds_grl[1]

        for l in range(len(cfg.GRL.PLACE_ADV)):
            if cfg.GRL.PLACE_ADV[l] == 'Y':
                # reshape the features (e.g. 128x5x2 --> 640x2)
                pred_source = preds_source[l]
                pred_target = preds_target[l]
                # prepare domain labels

                if cfg.GRL.ENTROPY:
                    loss_grl += entropy_loss(pred_source)
                    loss_grl += entropy_loss(pred_target)

                else:
                    label_source = torch.zeros(pred_source.size(0)).long()
                    label_target = torch.ones(pred_target.size(0)).long()

                    label_domain = torch.cat((label_source, label_target), 0)
                    label_domain = label_domain.cuda(non_blocking=True)
                    pred_domain = torch.cat((pred_source, pred_target), 0)

                    loss_grl += loss_fun_grl(pred_domain, label_domain)

        loss = loss_action + loss_grl
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if writer is not None:
                writer.add_scalars(
                    {"Train/loss_joint": loss, "Train/loss_action": loss_action, "Train/loss_grl": loss_grl, "Train/lr": lr},
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None, writer_str="Val/mAP"):
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(val_loader):
        ...
        if cfg.DETECTION.ENABLE:
            # duplicate inputs
            global_inputs = [torch.cat((inputs[i], inputs[i]), dim=0) for i in range(len(inputs))]
            # Compute the predictions.
            _, preds = model(global_inputs, bboxes_source=meta["boxes"], bboxes_target=meta["boxes"], head_type='grl_action')
