...
### Training 
COUT_DIR=/cluster/work/cvl/gusingh/data/ava-kinetics/kinetics/experiments/SLOWFAST_32x2_R50_MIT_YOLO_DA_ROT_05/
CFG_FILE=configs/AVA-KIN/KIN/SLOWFAST_32x2_R50_YOLO_DA.yaml
python tools/run_net.py --cfg ${CFG_FILE} AVA.FRAME_DIR ${TARGET_DIR} AUX.FRAME_DIR ${TARGET_DIR}  AUX.LOSS_FACTOR ${LOSS_FACTOR} AVA.DETECTION_SCORE_THRESH ${TRAIN_THRESH} OUTPUT_DIR ${COUT_DIR} AUX.AUX_TYPE rotation AUX.CLASSES 6
now=$(date +"%T")
echo "time after  DA training to AV with Kinetics Source: $now"
...
now=$(date +"%T")
echo "time after testing DA AVA supervised Kinetics on AVA: $now"
echo ${TARGET_DIR}

CFG_FILE=configs/AVA-KIN/KIN/SLOWFAST_32x2_R50_YOLO.yaml
python tools/run_net.py --cfg ${CFG_FILE}  AVA.FRAME_DIR ${TARGET_DIR} TRAIN.ENABLE False TENSORBOARD.ENABLE False AVA.DETECTION_SCORE_THRESH ${EVAL_THRESH} OUTPUT_DIR ${COUT_DIR}
now=$(date +"%T")
echo "time after testing DA AVA supervised Kinetics on KIN: $now"
echo ${TARGET_DIR}
...
python tools/run_net.py --cfg ${CFG_FILE}  AVA.FRAME_DIR ${TARGET_DIR} TRAIN.ENABLE False TENSORBOARD.ENABLE False AVA.DETECTION_SCORE_THRESH ${EVAL_THRESH} OUTPUT_DIR ${COUT_DIR}
now=$(date +"%T")
echo "time after testing DA AVA supervised Kinetics on AVA-KIN: $now"
echo ${TARGET_DIR}
