...
def inflate_weight(state_dict_2d, state_dict_3d):
    ...
    for k, v2d in state_dict_2d.items():
        ...
        if len(v2d.shape) == 4 and len(v3d.shape) == 5:
            ...
            # assert v2d.shape[-2:] == v3d.shape[-2:], v2d.shape
            min0 = min(v2d.shape[0], v3d.shape[0])
            min1 = min(v2d.shape[1], v3d.shape[1])
            min3, min4 = min(v2d.shape[2], v3d.shape[3]), min(v2d.shape[3], v3d.shape[4])
            uv2d = v2d.unsqueeze(2).repeat(1, 1, v3d.shape[2], 1, 1) / v3d.shape[2]
            
            v3d[:min0,:min1,:,:min3,:min4] = uv2d[:min0,:min1,:,:min3,:min4]

            # if v2d.shape[:2] != v3d.shape[:2]:
            #     print("Unexpected {}: {} -|> {}: {}".format(k, v2d.shape, k, v3d.shape))
            # if v2d.shape[0] != v3d.shape[1]:
            #     print("Unexpected {}: {} -|> {}: {}".format(k, v2d.shape, k, v3d.shape))
            

        elif v2d.shape == v3d.shape:
            v3d = v2d
        elif len(v2d.shape) == len(v3d.shape) == 1:
            min0 = min(v2d.shape[0], v3d.shape[0])
            v3d[:min0] = v2d[:min0]
        else:
            logger.info(
                "Inflate Unexpected {}: {} -|> {}: {}".format(
                    k, v2d.shape, k, v3d.shape
                )
            )
        state_dict_inflated[k] = v3d.clone()
    
    for k, v3d in state_dict_3d.items():
        if k.find('rcu.conv_whh.weight')>-1:
            v3d = v3d.clone()*0.0
            for i in range(v3d.size(0)):
                v3d[i, i, :, :, :] = 1.0/(v3d.size(2)*v3d.size(3)*v3d.size(4))
            print('conv_whh sum',v3d.sum())
            state_dict_inflated[k] = v3d
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        if inflation:
            logger.info('Try to inflate the model')
            checkpoint["model_state"] = inflate_weight(
                checkpoint["model_state"], model_state_dict_3d
            )
        ...
        # print('Loaded checkpoint state dict', pre_train_dict.keys())
        ...
        pre_train_dict_not_match = {
            k: v
            for k, v in pre_train_dict.items()
            if k not in model_dict}
        
        for k in pre_train_dict_not_match:
            logger.info("Checkpoint weights {} not used in current model.".format(k))
...
def load_train_checkpoint(cfg, model, optimizer, epoch_reset=False):
    """
