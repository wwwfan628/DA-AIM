...
def compute_precision_recall(scores, labels, num_gt):
  ...
  """
    ...
    true_positive_labels = labels[sorted_indices]    # descending
    false_positive_labels = 1 - true_positive_labels
    cum_true_positives = np.cumsum(true_positive_labels)    # ascending
    cum_false_positives = np.cumsum(false_positive_labels)
    precision = cum_true_positives.astype(float) / (
        cum_true_positives + cum_false_positives
    )
    recall = cum_true_positives.astype(float) / num_gt      # ascending
    threshold = scores[sorted_indices]    # descending
