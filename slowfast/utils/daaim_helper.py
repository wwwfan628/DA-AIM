...
def mix(inputs, inputs_unlabeled, boxes, boxes_unlabeled, labels, pseudo_labels_unlabeled, pseudo_logits_unlabeled, cfg, writer=None):
    ...
    for i in range(batch_size):
        ...
        if num_boxes_i_without_duplicate == 0 and (not cfg.DAAIM.PSEUDO_LABEL_ENABLE):
    ...
    if isinstance(inputs, (list,)) and len(inputs) > 1:  # slowfast inputs, 2 threads
        ...
        resized_boxes_selected, masks_slow_labeled, masks_fast_labeled = createLabeledMasks(inputs, boxes_selected, resize_labeled_flags, resize_enable=cfg.DAAIM.RESIZE_ENABLE)
        # mix frames
        inputs_mix = mixFrames(inputs, inputs_unlabeled, resize_labeled_flags, masks_slow_labeled, masks_fast_labeled)
    else:
        # create masks for frames
        resized_boxes_selected, masks_slow_labeled = createLabeledMasks(inputs, boxes_selected, resize_labeled_flags, resize_enable=cfg.DAAIM.RESIZE_ENABLE)
        # mix frames
        inputs_mix = mixFrames(inputs, inputs_unlabeled, resize_labeled_flags, masks_slow_labeled)

    # mix boxes & labels
    if cfg.DAAIM.PSEUDO_TARGETS == 'binary':
    ...
    elif cfg.DAAIM.PSEUDO_TARGETS == 'logits':
    ...
    if cfg.TENSORBOARD.DAAIM.PLOT_SAMPLES and (writer is not None):
        global batch_idx
        if batch_idx % 1 == 0:
...
def mixTargetsAndBoxes(boxes, boxes_unlabeled, resized_boxes_selected, boxes_selected, labels, pseudo_targets_unlabeled, pseudo_logits_unlabeled, inputs, cfg):
    ...
    if cfg.DAAIM.PSEUDO_LABEL_ENABLE:
        ...
        for i in range(batch_size):
            ...
            if cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'none':
                unlabeled_weight = 1.0
            elif cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'uniform':
                unlabeled_weight = 0.2
            elif cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'threshold_uniform':
                thresholds = torch.tensor(cfg.DAAIM.THRESHOLDS).unsqueeze(dim=0).repeat_interleave(len(pseudo_logits_i_unlabeled), dim=0).cuda()
            ...
            elif cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'threshold':
                thresholds = torch.tensor(cfg.DAAIM.THRESHOLDS).unsqueeze(dim=0).repeat_interleave(len(pseudo_logits_i_unlabeled), dim=0).cuda()
                unlabeled_weight = pseudo_logits_i_unlabeled.ge(thresholds).float()
            elif cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'ce_none':
                unlabeled_weight = 1.0
            elif cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'ce_uniform':
                unlabeled_weight = 0.2
            elif cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'ce_threshold_uniform':
                max_pseudo_logits_i_unlabeled, _ = torch.max(pseudo_logits_i_unlabeled, dim=1)
                unlabeled_weight = torch.sum(max_pseudo_logits_i_unlabeled.ge(cfg.DAAIM.THRESHOLD).long() == 1).item() / max_pseudo_logits_i_unlabeled.numel()
            elif cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'ce_threshold':
                max_pseudo_logits_i_unlabeled, _ = torch.max(pseudo_logits_i_unlabeled, dim=1)
                unlabeled_weight = max_pseudo_logits_i_unlabeled.ge(cfg.DAAIM.THRESHOLD).float().cuda()
            ...
            if cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'threshold' or cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'ce_threshold':
                boxes_weights_i_nonoverlap = unlabeled_weight[nonoverlap_indices]
            elif cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'none' or cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'threshold_uniform' or \
                    cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'uniform':
                boxes_weights_i_nonoverlap = unlabeled_weight * torch.ones([len(boxes_i_nonoverlap), num_classes]).cuda()
            elif cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'ce_threshold_uniform' or cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'ce_uniform'\
                    or cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'ce_none':
...
def use_pseudo_labels(inputs_aux, boxes_aux, pseudo_logits_unlabeled, pseudo_labels_unlabeled, cfg):
    ...
    if cfg.DAAIM.CONSISTENCY_LOSS_WEIGHT == 'none':
        ...
        for i in range(batch_size):
            ...
            thresholds = torch.tensor(cfg.DAAIM.THRESHOLDS).unsqueeze(dim=0).repeat_interleave(len(boxes_i_mix), dim=0).cuda()
        ...
        for i in range(batch_size):
            ...
            unlabeled_weight = torch.sum(max_pseudo_logits_i_mix.ge(cfg.DAAIM.THRESHOLD).long() == 1).item() / max_pseudo_logits_i_mix.numel()
        ...
        for i in range(batch_size):
            max_pseudo_logits_i_mix = max_pseudo_logits_mix[boxes_mix[:, 0] == i]
            boxes_weights_i_mix = max_pseudo_logits_i_mix.ge(cfg.DAAIM.THRESHOLD).float().cuda()
            boxes_weights_mix.append(boxes_weights_i_mix)
        boxes_weights_mix = torch.cat(boxes_weights_mix)
    return inputs_mix, boxes_mix, pseudo_targets_mix, boxes_weights_mix
