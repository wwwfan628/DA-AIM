...
def get_ava_mini_groundtruth(full_groundtruth, denser=False):
    """
    ...
    for i in range(3):
        for key in full_groundtruth[i].keys():
            # print('meters.py line 43')
            # print('key', key)
            video_name = key.split(",")[0]
            sec = int(float(key.split(",")[1]))
            ...
            if denser and format_correct and sec % 1 == 0:
                ret[i][key] = full_groundtruth[i][key]
            elif sec<10 or sec % 4 == 0:
                ret[i][key] = full_groundtruth[i][key]
    return ret
...
class AVAMeter(object):
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        elif (self.mode == "val") or (self.mode == "aux_val"): # luca added da_val
            stats = {
                "_type": "{}_iter".format(self.mode),
                "cur_epoch": "{}".format(cur_epoch + 1),
                "cur_iter": "{}".format(cur_iter + 1),
    ...
    def finalize_metrics(self, log=True, writer=None, cur_epoch=0):
        """
        ...
        if self.mode == "test" or (self.full_ava_test and ((self.mode == "val") or (self.mode == "aux_val"))): #luca
            # added da_val
    ...
    def log_epoch_stats(self, cur_epoch, writer=None):
        ...
        Args:
            ...
            writer(tensorboard.writer): to save fig.
        """
        if self.mode in ["val", "da_val", "test"]:
            self.finalize_metrics(log=False, writer=writer, cur_epoch=cur_epoch)
            stats = {
...
class DA_AVAMeter(AVAMeter):
    ...
    def __init__(self, overall_iters, cfg, mode):
        ...
        self.lr = None
        self.loss = ScalarMeter(cfg.LOG_PERIOD)
        self.full_ava_test = cfg.AUX.FULL_TEST_ON_VAL
        self.mode = mode
        self.iter_timer = Timer()
        self.data_timer = Timer()
        self.net_timer = Timer()
        ...
        self.overall_iters = overall_iters
        self.excluded_keys = read_exclusions(
            os.path.join(cfg.AUX.ANNOTATION_DIR, cfg.AUX.EXCLUSION_FILE)
        )
        self.categories, self.class_whitelist = read_labelmap(
            os.path.join(cfg.AUX.ANNOTATION_DIR, cfg.AUX.LABEL_MAP_FILE)
        )
