...
def make_image_key(video_id, timestamp, denser=False):
    """Returns a unique identifier for a video id & timestamp."""
    # print('ava_eval_helper.py line 55')
    # print('video_id', video_id)
    cad1_rex = re.compile("^CADU[0-9]{4}$")
    cad2_rex = re.compile("^C0[0-9]{3}$")
    format_correct = cad1_rex.match(video_id) or cad2_rex.match(video_id)
    if denser and format_correct:
        return "%s,%04d" % (video_id, int(round(float(timestamp) * 100)))
    else:
...
def read_csv(csv_file, class_whitelist=None, load_score=False, denser=False):
    ...
    with g_pathmgr.open(csv_file, "r") as f:
        ...
        for row in reader:
            ...
            image_key = make_image_key(row[0], row[1], denser)
...
def read_exclusions(exclusions_file):
    ...
    print('Number of excluded are', len(excluded))
    return excluded
...
def evaluate_ava(
    ...
    # Luca Sieber
    if cfg.OUTPUT_DIR != "":
        write_results(detections, os.path.join(cfg.OUTPUT_DIR,  "detections_%s.csv" % name))
    ...
    results = run_evaluation(cfg, categories, groundtruth, detections, excluded_keys, writer=writer, cur_epoch=cur_epoch)
...
def run_evaluation(
    cfg, categories, groundtruth, detections, excluded_keys, verbose=True, writer=None, cur_epoch=0,
):
    ...
    if cfg.TENSORBOARD.DAAIM.PLOT_PRECISION_RECALL_CURVE and (writer is not None):
        ...
        for idx in range(len(precisions_per_class)):
            if (idx + 1) in category_index:
                class_name = category_index[idx + 1]["name"]
                precision = precisions_per_class[idx]
                recall = recalls_per_class[idx]
                threshold = thresholds_per_class[idx]
                plot_precision_recall_curve(precision, recall, threshold, class_name=class_name, writer=writer, cur_epoch=cur_epoch)
    return metrics


def plot_precision_recall_curve(precision, recall, threshold, class_name=None, writer=None, cur_epoch=0):
    ...
    # find local maximum of f1 score
    max_f1_precision, max_f1_recall, max_f1_threshold = find_max_f1_score(precision, recall, threshold)

    # draw precision-recall curve
    fig, ax_1 = plt.subplots(figsize=(7, 5), nrows=1, ncols=1)
    ax_2 = ax_1.twinx()
    fig.patch.set_facecolor('w')
    ax_1.grid(linestyle='dotted')
    ax_1.set_facecolor('whitesmoke')
    ax_1.set_ylabel('Precision')
    ax_2.set_ylabel('Threshold')
    ax_1.set_xlabel('Recall')
    ax_1.plot(recall, precision, 's-', color='b', label='precision')
    ax_2.plot(recall, threshold, 's-', color='g', label='threshold')
    ax_1.title.set_text('Precision-Recall Curve: ' + class_name)
    ...
    for i in range(len(max_f1_precision)):
        x = max_f1_recall[i]
        y_min = 0
        y_1 = max_f1_precision[i]
        y_2 = max_f1_threshold[i]
        ax_1.plot(x, y_1, marker='o', markersize=5, color="red")
        ax_2.plot(x, y_2, marker='o', markersize=5, color="red")
        ax_1.vlines(x, y_min, y_1, linestyles='dashdot', colors='red')
        ax_2.vlines(x, y_min, y_2, linestyles='dashdot', colors='red')
        print('{} maximum of F1 scores:'.format(i))
        print('precision: ', y_1)
        print('recall: ', x)
        print('threshold: ', y_2)
    tag = str(cur_epoch) + '_epoch_' + class_name
    # fig.savefig(path)
...
def find_max_f1_score(precision, recall, threshold):
    # find max value of f1 score
    f1 = 2 * (precision * recall) / (precision + recall)
    index = (f1 == np.nanmax(f1))
    max_f1_precision = precision[index]
    max_f1_recall = recall[index]
    max_f1_threshold = threshold[index]
    return max_f1_precision, max_f1_recall, max_f1_threshold
