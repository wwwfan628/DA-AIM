...
class ResNetRoIHeadGRL(nn.Module):
    """
    ResNe(X)t RoI head.
    """
    ...
    def __init__(
        ...
        """
        super(ResNetRoIHeadGRL, self).__init__()
        #########################################
        # added by jizhao
        self.beta = beta
        ##########################################
        ...
        ###############################################################
        # added by jizhao
        self.projection_grl = head_helper_grl.Video_Head(dropout_i=dropout_i, dropout_v=dropout_v, add_fc=add_fc,
                                                      share_params=share_params, feat_shared_dim=feat_shared_dim,
                                                      adv=adv, use_attn=use_attn, use_attn_frame=use_attn_frame)
        ###############################################################

    def forward(self, inputs_source=None, inputs_target=None, bboxes_source=None, bboxes_target=None, head_type='action'):
        ...
        for pathway in range(self.num_pathways):
            ...
            out = roi_align(out, bboxes_source)
        ...
        x = self.projection(x)
        if head_type.find('ce') == -1:
            x = self.act(x)

        ############################
        # added by jizhao
        y = self.projection_grl(input_source=inputs_source, input_target=inputs_target, beta=self.beta)
...
class ResNetRoIHead(nn.Module):
    ...
    def __init__(
        ...
        if aux_classes > 1:
            self.projection_aux = nn.Linear(sum(dim_in), aux_classes, bias=True)
            self.act_aux = nn.Softmax(dim=1)

    def forward(self, inputs, bboxes, head_type='action', numb=1):
        assert (
            len(inputs) == self.num_pathways
        ), "Input tensor does not contain {} pathway".format(self.num_pathways)
        pool_out = []
        for pathway in range(self.num_pathways):
            # print('\n\n\n HEAD inputs {} \n\n\n '.format(inputs[pathway].shape))
            t_pool = getattr(self, "s{}_tpool".format(pathway))
            out = t_pool(inputs[pathway])
        ...
        # to record final feature map
        self.final_feature_map = features.clone().detach().view(features.shape[0], -1)
        ...
        # Luca Sieber addition for domain adaptation
        if head_type.find('aux')>-1 and self.aux_classes>1:
            y = self.projection_aux(x)
            # y = self.act_aux(y)
        
        if head_type.find('action')>-1:
            x = self.projection(x)
            if head_type.find('ce')==-1:
                x = self.act(x)
        ...
        if head_type.find('aux')>-1 and head_type.find('action')>-1:
            return x, y
        elif head_type.find('aux')>-1:
            return y
        else:
            return x
