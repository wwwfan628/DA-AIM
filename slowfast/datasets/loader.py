...
def construct_loader_da(cfg, split, is_precise_bn=False):
    ...
    assert split in ["aux_train", "aux_val"]
    if split in ["aux_train"]:
        dataset_name = cfg.AUX.DATASET
        batch_size = int(cfg.TRAIN.BATCH_SIZE / max(1, cfg.NUM_GPUS))
        shuffle = True
        drop_last = True
    elif split in ["aux_val"]:
        dataset_name = cfg.AUX.DATASET
        batch_size = int(cfg.TRAIN.BATCH_SIZE / max(1, cfg.NUM_GPUS))
        shuffle = False
        drop_last = False
    ...
    if cfg.MULTIGRID.SHORT_CYCLE and split in ["aux_train"] and not is_precise_bn:
        # Create a sampler for multi-process training
        sampler = utils.create_sampler(dataset, shuffle, cfg)
        batch_sampler = ShortCycleBatchSampler(
            sampler, batch_size=batch_size, drop_last=drop_last, cfg=cfg
