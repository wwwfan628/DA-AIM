...
def read_binary(rel_path, hfile):

    return hfile[rel_path]['raw'][0]

def read_image(rel_path, hfile):
    bytes = read_binary(rel_path, hfile)
...
def retry_load_images(image_paths, frame_dir, retry=10, backend="pytorch"):
    """
    ...
    """
    # hfile = h5py.File(frame_dir+'.hdf5', 'r')
    for i in range(retry):
        imgs = []
        for image_path in image_paths:
            full_path = os.path.join(frame_dir, image_path)
            with g_pathmgr.open(full_path, "rb") as f:
               img_str = np.frombuffer(f.read(), np.uint8)
               img = cv2.imdecode(img_str, flags=cv2.IMREAD_COLOR)
            #print(full_path, image_path, img1.shape)
            # pil_image = read_image(image_path, hfile)
            ...
            #diff = np.abs(img1-img)
            #print(diff[99,99,:], diff[199,199,:])
            #diff = diff.flatten()
            #print(full_path, image_path, img1.shape, img.shape, np.sum(diff), np.std(diff), np.max(diff), diff[:9])
            #image_path = image_path.split('/')[1]
            #print(image_path)
            #img = img[:,:,0].flatten()
            #img1 = img1[:,:,0].flatten()
            #print(np.mean(img),np.max(img),np.std(img),np.mean(img1),np.max(img1),np.std(img1))
            #cv2.imwrite('frame'+image_path, img1)
            #cv2.imwrite('hdf5'+image_path,img)
        if all(img is not None for img in imgs):
            ...
            #print('hdf5 is being reused',img.shape)
