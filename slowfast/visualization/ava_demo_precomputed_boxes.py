...
class AVAVisualizerWithPrecomputedBox:
    ...
    def __init__(self, cfg):
        ...
        if g_pathmgr.isdir(self.source):
            ...
            dataset_name = self.source.split("/")[-3]
            if dataset_name == 'ava':
            ...
            elif dataset_name == 'armasuisse':
                self.source = os.path.join(
                    self.source, "img_%05d.jpg"
                )
        else:
            self.video_name = self.source.split("/")[-1]
    ...
    def draw_video(self):
        ...
        for idx, keyframe_idx in enumerate(tqdm.tqdm(all_keys)):
            ...
            for i, boxes in enumerate(pred_gt_boxes):
                ...
                # delete duplicated boxes
                boxes = merge_duplicate(boxes)
...
def load_boxes_labels(cfg, video_name, fps, img_width, img_height):
    ...
    dataset_name = g_pathmgr.get_local_path(path=cfg.DEMO.INPUT_VIDEO).split("/")[-3]

    def sec_to_frameidx(sec):
        sec = float(sec)
        if dataset_name == 'armasuisse':
    ...
    def process_bboxes_dict(dictionary, timestamps):
        """
        ...
        for keyframe_sec, boxes_and_labels in dictionary.items():
            time_stamp = timestamps[keyframe_sec]
            # Ignore keyframes with no boxes
            if len(boxes_and_labels) == 0:
                continue
            keyframe_idx = sec_to_frameidx(time_stamp)
    ...
    preds_boxes, preds_timestamps, _, _ = parse_bboxes_file(
    ...
    preds_timestamps = preds_timestamps[video_name]
    if gt_boxes_path == "":
        gt_boxes = None
    else:
        gt_boxes, gt_timestamps, _, _ = parse_bboxes_file(
        ...
        gt_timestamps = gt_timestamps[video_name]
    preds_boxes = process_bboxes_dict(preds_boxes, preds_timestamps)
    if gt_boxes is not None:
        gt_boxes = process_bboxes_dict(gt_boxes, gt_timestamps)
...
def area(box):
  ...
  Args:
    box (tensor): shape [1, 4], represents (x1, y1, x2, y2)
  Returns:
...
def intersection(box1, box2):
  ...
  Args:
    box1 (tensor): shape [1, 4], represents (x1, y1, x2, y2) of box1
    box2 (tensor): shape [1, 4], represents (x1, y1, x2, y2) of box2
  Returns:
...
def isBoxDuplicate(box1, box2):
    ...
    duplicate = ((intersection_area / area_box1) >= 0.5) or ((intersection_area / area_box2) >= 0.5)
    return duplicate


def merge_duplicate(boxes):
    # print('ava_demo_precomputed_boxes.py line 543!!!')
    # print('boxes', boxes)
    bboxes = boxes[1]
    already_in_group = [False for _ in range(len(bboxes))]
    bboxes_groups_indices = []
    bboxes_groups = []
    bboxes_new = []
    labels_new = []
    for i, bbox_1 in enumerate(bboxes):
        if area(bbox_1) <= 0:
            ...
            if not already_in_group[i]:    # box hasn't been checked
                ...
                for j, bbox_2 in enumerate(bboxes):
                    if area(bbox_2) <= 0:
    ...
    if bboxes_groups:
        for i in range(len(bboxes_groups)):
            bbox_group = torch.tensor(bboxes_groups[i]).mean(dim=0).tolist()
            print(boxes)
            label_group = boxes[2][bboxes_groups_indices[i]].mean(dim=0)
            bboxes_new.append(bbox_group)
            labels_new.append(label_group)
        labels_new = torch.stack(labels_new)
        boxes_new = [boxes[0], bboxes_new, labels_new]
        # print('boxes_new', boxes_new)
        return boxes_new
    else:
        boxes_new = [boxes[0], [], torch.zeros([0, boxes[2].shape[1]])]
        # print('boxes_new', boxes_new)
        return boxes_new
